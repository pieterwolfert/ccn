...
def run(X_train, X_test, y_train, y_test, title, filetitle):
    """
    Main function for training and testing the rnn.
    ...
    title -- title of task
    filetitle -- title for image plot
    """
    # training phase
    rnn_b = Model(word_dim, hidden_dim)
    rnn_f = Model(word_dim, hidden_dim, mode='fa')
    rnn_df = Model(word_dim, hidden_dim, mode='dfa')
    b_loss = rnn_b.train(X_train, y_train, learning_rate=0.05, nepoch=nrepoch, evaluate_loss_after=1)
    f_loss = rnn_f.train(X_train, y_train, learning_rate=0.05, nepoch=nrepoch, evaluate_loss_after=1)
    df_loss = rnn_df.train(X_train, y_train, learning_rate=0.05, nepoch=nrepoch, evaluate_loss_after=1)
...
if __name__ == "__main__":
    ...
    np.random.seed(10)
    run(X_train, X_test, y_train, y_test, reddit_title, 'reddit')
